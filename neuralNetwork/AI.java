...
public class AI {
	...
	public void train(INDArray inputs, int action) {
		...
		// As it happens if it gets stuck on a loop and if the score goes below integer
		// limit it will not work anymore.
	...
	/**
	 * Calculates a score for the action that the AI chose We dont know if the
	 * action was 100% correct either so it will calculate the score by looking if
	 * it moved closer to the apple which is good or ate the apple Or if it died
	 * because of this action then it was a bad move With this the AI will learn the
	 * basics of the game which is to eat apples and not die
	 */
	public double getScore(int action) {
		...
		// There are often times when its required to go to the opposite direction so it
		// will not punish for it that much
		...
		if (willDieAtPosition(futurePosition)) {
			score -= 2000 - SnakeNode.list.size() * 10;
		}
	...
	/**
	 * Gets the inputs. This is what the AI will see With these it can see the
	 * distance to the apple and if it will die by moving to any of the 4 directions
	 */
	public INDArray getInputs() {
		INDArray inputs = Nd4j.create(1, 19);
		...
		inputs.getRow(0).getColumn(6).assign(
				willDieAtPosition(new Position(Snake.snakeHead.position.x + 1, Snake.snakeHead.position.y + 1)));
		inputs.getRow(0).getColumn(7).assign(
				willDieAtPosition(new Position(Snake.snakeHead.position.x + 1, Snake.snakeHead.position.y - 1)));
		inputs.getRow(0).getColumn(8).assign(
				willDieAtPosition(new Position(Snake.snakeHead.position.x - 1, Snake.snakeHead.position.y + 1)));
		inputs.getRow(0).getColumn(9).assign(
				willDieAtPosition(new Position(Snake.snakeHead.position.x - 1, Snake.snakeHead.position.y - 1)));
		inputs.getRow(0).getColumn(10).assign(
				willDieAtPosition(new Position(Snake.snakeHead.position.x, Snake.snakeHead.position.y + 2)));
		inputs.getRow(0).getColumn(11).assign(
				willDieAtPosition(new Position(Snake.snakeHead.position.x, Snake.snakeHead.position.y - 2)));
		inputs.getRow(0).getColumn(12).assign(
				willDieAtPosition(new Position(Snake.snakeHead.position.x + 2, Snake.snakeHead.position.y)));
		inputs.getRow(0).getColumn(13).assign(
				willDieAtPosition(new Position(Snake.snakeHead.position.x - 2, Snake.snakeHead.position.y)));
		inputs.getRow(0).getColumn(14).assign(
				willDieAtPosition(new Position(Snake.snakeHead.position.x, Snake.snakeHead.position.y + 3)));
		inputs.getRow(0).getColumn(15).assign(
				willDieAtPosition(new Position(Snake.snakeHead.position.x, Snake.snakeHead.position.y - 3)));
		inputs.getRow(0).getColumn(16).assign(
				willDieAtPosition(new Position(Snake.snakeHead.position.x + 3, Snake.snakeHead.position.y)));
		inputs.getRow(0).getColumn(17).assign(
				willDieAtPosition(new Position(Snake.snakeHead.position.x - 3, Snake.snakeHead.position.y)));
		inputs.getRow(0).getColumn(18).assign(Snake.direction.ordinal());
		...
		MultiLayerConfiguration conf = new NeuralNetConfiguration.Builder().seed(12345)
				...
				.layer(5,
						new DenseLayer.Builder().nIn(hiddenLayers).nOut(hiddenLayers).weightInit(WeightInit.XAVIER)
								.activation(Activation.RELU).build())
				.layer(6,
						new OutputLayer.Builder(LossFunctions.LossFunction.MSE).nIn(hiddenLayers).nOut(outputNum)
