...
public class Snake extends JPanel {
	...
	/**
	 * Set if it will train it or use the pretrained model. Note: if this is enabled
	 * it will delete the previously saved model and overwrite it The model is saved
	...
	public void doAction() {
		...
		for (SnakeNode node : list) {
			...
			if (!node.equals(snakeHead) && isGoingToDieAtLocation(node.position) || node.position.x < 0
					|| node.position.x > Main.instance.getWidth() || node.position.y < 0
					|| node.position.y > Main.instance.getHeight() || movesWithNoAppleEaten > 100) {
				games++;
...
//				}
	...
	public void paintComponent(Graphics graphics) {
		...
		for (SnakeNode node : list) {
			...
			g.fillRect(node.position.x, node.position.y, snakeHead.size, snakeHead.size);
		}
	...
	public static boolean isGoingToDieAtLocation(Position position) {
		return snakeHead.position.x == position.x && snakeHead.position.y == position.y || position.x < 0
				|| position.x > Main.instance.getWidth() || position.y < 0 || position.y > Main.instance.getHeight()
				|| movesWithNoAppleEaten > 300;
	}
